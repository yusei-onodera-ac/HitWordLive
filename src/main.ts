...
function currentDisplay() {
  ...
  if (state.winner) return { chars: [...state.winner.answer], label: '正解', result: `${length}hit 0blow`, mark: 'correct' };
  if (state.phase === 'answer' && state.answer) return { chars: [...state.answer], label: '時間切れ', result: '答え表示', mark: 'wrong' };
  if (latest) return { chars: [...latest.answer], label: '不正解', result: `${latest.hits}hit ${latest.blows}blow`, mark: 'wrong' };
  return { chars: Array.from({ length }, () => '?'), label: '', result: '', mark: 'none' };
}

function historyColumns() {
  const placeholders = Array.from({ length: Math.max(0, 10 - state.history.length) }, (_, i) => ({ id: `blank-${i}`, user: 'ひよこ', answer: '', hits: 1, blows: 0 }));
  const rows = [...state.history, ...placeholders].slice(0, 10);
  return rows.map((item) => `<p><span>コメント名：${escapeHtml(item.user)}</span><b>${item.hits}hit ${item.blows}blow</b></p>`).join('');
}
  ...
  root.innerHTML = `
    <main class="overlay-stage ${display.mark === 'correct' ? 'is-correct' : ''}">
      ...
      <section class="question-area">
        <p class="prompt">単語を当てろ（${state.length || display.chars.length}文字）</p>
        <div class="letter-row">
          ${display.chars.map((char) => `<div class="letter-box">${escapeHtml(char)}</div>`).join('')}
        </div>
        ${display.mark !== 'none' ? `<div class="judge ${display.mark}"><span>${display.label}</span><strong>${display.result}</strong></div>` : ''}
        ${display.mark === 'wrong' ? '<div class="wrong-mark">×</div>' : ''}
        ${display.mark === 'correct' ? '<div class="correct-mark">〇</div>' : ''}
      </section>
      ${state.winner ? `<section class="winner-strip"><span>解答履歴</span><strong>正解者　${escapeHtml(state.winner.user)}　${state.winner.point}ポイント獲得</strong></section>` : '<h2 class="history-title">解答履歴</h2>'}
      <section class="history-grid">${historyColumns()}</section>
    </main>`;
}
